...
export default function UnifAIApp() {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      try {
        ...
        // Load tools and categories (available to all) in background
        loadPublicData();
      } catch (err) {
    ...
    // Start initialization immediately (don't block splash)
    initialize();
  ...
  const handleSplashComplete = () => {
    console.log('Splash complete - moving to home');
    setShowSplash(false);
  ...
  const handleOpenTool = async (tool) => {
    ...
    // Track usage in background (only if tool has valid UUID)
    if (user?.id && tool?.id && typeof tool.id === 'string' && tool.id.length > 10) {
      try {
        await historyService.trackToolUsage(user.id, tool.id);
      // Reload recent tools
