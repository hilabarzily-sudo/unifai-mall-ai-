...
export function transformTool(dbTool: any) {
  if (!dbTool) return null
  
  // Extract categories
  const categories = dbTool.category_tools?.map((ct: any) => ct.category) || []
  const primaryCategory = categories[0]?.name_he || categories[0]?.name_en || 'כללי'
  
  // Extract tags
  const tags = dbTool.tool_tags?.map((tt: any) => tt.tag?.name_he) || []
...
export function transformTools(dbTools: any[]) {
  if (!dbTools || !Array.isArray(dbTools)) return []
  return dbTools.map(transformTool).filter(Boolean)
}

// Transform category
export function transformCategory(dbCategory: any) {
...
export function transformPrompt(dbPrompt: any) {
...
export function transformExample(dbExample: any) {
...
export function groupToolsByCategory(tools: any[], categories: any[]) {
